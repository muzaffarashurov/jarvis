"""Electricity consumption PersonalDataSource for EP-093.

Implements EP-092's `PersonalDataSource` contract directly (STEP 1
design §6.2, Design A -- no separate acquisition-provider
abstraction). This module owns 100% of the V1 manual-entry and
CSV-import logic for the `electricity_consumption` category: reading
and writing its own local, append-only JSONL log
(`data/database/personal_data_electricity_gas/electricity_consumption.jsonl`
by default -- deliberately outside `data/database/personal_data/`,
which is EP-092's exclusive persisted-output space) and parsing the
canonical EP-093 CSV format (`date,value,unit,meter_id`).

This module never touches `PersonalDataProvider`,
`PersonalDataPersistence`, `PersonalDataRegistry`, or `CommandResult`
directly (STEP 1 design §11's layering rule) -- it only produces
`PersonalDataPoint`s for `PersonalDataManager` to collect via
`collect()`. Deduplication, consent, and actual EP-092 persistence
are entirely EP-092's (`store_if_new()`, unchanged) -- this module
introduces no second dedup mechanism.
"""
...
CATEGORY: str = "electricity_consumption"
SOURCE_ID: str = "electricity_local_entry"
DEFAULT_LOG_PATH: Path = Path("data/database/personal_data_electricity_gas") / f"{CATEGORY}.jsonl"

_REQUIRED_CSV_COLUMNS = ("date", "value", "unit")


class ElectricityMeterReadingError(Exception):
    """Raised for an unrecoverable electricity-reading operation (EP-093).
    ...
    this exception (STEP 1 design §6.4's "invalid-row handling" rule).
    """
...
def _parse_value(raw: str) -> float:
    ...
    Raises:
        ValueError: If `raw` is not a valid decimal number, or is
            ...
            is never a valid electricity measurement, per EP-093 STEP
            3 audit finding EP093-AUDIT-001).
    """
    try:
        parsed = float(raw)
...
def _parse_timestamp(raw: str | None) -> datetime:
    ...
    date, per STEP 1 design §6.4. An empty/None value means "now."
...
def _build_point(
    ...
    """Validate raw fields and construct one electricity `PersonalDataPoint`.

    Raises:
        ValueError: If `value` is not numeric, `unit` is empty, or
            `timestamp_raw` is not empty/valid ISO-8601 (STEP 1 design
            §6.4's "validation" rule).
    """
...
def _append_to_log(point: PersonalDataPoint, log_path: Path) -> None:
    """Append `point` to `log_path` as one JSONL line (EP-093's own log, not EP-092's)."""
...
def append_electricity_reading(
    value: str,
    unit: str,
    timestamp: str | None = None,
    log_path: Path | None = None,
) -> PersonalDataPoint:
    """Validate and append one manually-entered electricity reading.

    Used by the `personal_data record-reading` CLI action. This is a
    plain module-level function (not a method on `ElectricityCsvSource`)
    so `personal_data_module.py` can call it without importing the
    `PersonalDataSource` subclass itself, per STEP 1 design §11's
    layering rule ("may NOT import ... any EP-093 source class
    directly").
    ...
    Returns:
        ...
        `PersonalDataPoint`. Note this is EP-093's own local log, not
        yet EP-092's actual persisted storage -- a subsequent
        `PersonalDataService.collect(SOURCE_ID)` call (via
        `PersonalDataManager`/`ElectricityCsvSource.collect()`) is
        required for it to reach `JsonlPersonalDataProvider`.
...
def import_electricity_csv(
    path: str | Path, log_path: Path | None = None
) -> tuple[int, list[str]]:
    """Parse and append every valid row of an electricity readings CSV.

    Used by the `personal_data import-csv` CLI action. Follows the
    canonical EP-093 CSV format (STEP 1 design §6.4):
    `date,value,unit,meter_id` (`meter_id` optional). One malformed row
    is logged and skipped -- it must not abort the remaining rows.
    ...
    Raises:
        ElectricityMeterReadingError: If `path` does not exist or
            cannot be opened at all -- a failure that prevents *any*
            progress, per §6.4's "error reporting" rule. A per-row
            problem within an otherwise readable file is never raised.
    """
    csv_path = Path(path)
    try:
        file = csv_path.open("r", encoding="utf-8-sig", newline="")
    except OSError as exc:
        raise ElectricityMeterReadingError(f"Cannot read CSV file '{csv_path}': {exc}") from exc
    ...
    with file:
        ...
        if missing:
            raise ElectricityMeterReadingError(
        ...
        for row_number, row in enumerate(reader, start=2):  # header is row 1
            try:
                ...
                logger.error(f"Electricity CSV import: skipping {message} in '{csv_path}'.")
...
class ElectricityCsvSource(PersonalDataSource):
    """Manual-entry + CSV-import `PersonalDataSource` for `electricity_consumption`.

    `collect()` reads every entry from this source's local log
    (written by `append_electricity_reading()`/`import_electricity_csv()`)
    and returns all of them, every time it is called -- it keeps no
    ...
    `(source_id, category, timestamp)` (STEP 1 design §10's "repeated
    execution" rule), so re-collecting the same log entries never
    creates a duplicate persisted record.
    """
    ...
    def source_id(self) -> str:
        """Return this source's registration id ("electricity_local_entry")."""
        return SOURCE_ID

    @property
    def category(self) -> str:
        """Return this source's category ("electricity_consumption")."""
        return CATEGORY
        ...
        with self._log_path.open("r", encoding="utf-8") as file:
            for line_number, raw_line in enumerate(file, start=1):
                ...
                try:
                    ...
                    logger.error(
                        f"Electricity local-entry log: skipping malformed line "
