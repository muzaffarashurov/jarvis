"""Gas consumption PersonalDataSource for EP-093.

Mirrors `electricity_source.py` exactly, for the `gas_consumption`
category. See that module's docstring for the full architectural
rationale (STEP 1 design §6.2's Design A, §6.4's V1 scope, §11's
layering rule) -- not repeated here to avoid drift between two
copies of the same explanation; this module's *behavior* is
independently complete and does not import anything from
...
CATEGORY: str = "gas_consumption"
SOURCE_ID: str = "gas_local_entry"
...
class GasMeterReadingError(Exception):
    """Raised for an unrecoverable gas-reading operation (EP-093).
...
def _parse_value(raw: str) -> float:
    ...
    Raises:
        ValueError: If `raw` is not a valid decimal number, or is
            ...
            is never a valid gas measurement, per EP-093 STEP 3 audit
            finding EP093-AUDIT-001).
    """
    try:
        parsed = float(raw)
...
def _build_point(
    ...
    """Validate raw fields and construct one gas `PersonalDataPoint`.
...
def append_gas_reading(
    value: str,
    unit: str,
    timestamp: str | None = None,
    log_path: Path | None = None,
) -> PersonalDataPoint:
    """Validate and append one manually-entered gas reading.

    Used by the `personal_data record-reading` CLI action. This is a
    plain module-level function (not a method on `GasCsvSource`) so
    ...
    Args:
        ...
        unit: The reading's unit (e.g. "m3").
    ...
    Returns:
        ...
        `PersonalDataManager`/`GasCsvSource.collect()`) is required
...
def import_gas_csv(path: str | Path, log_path: Path | None = None) -> tuple[int, list[str]]:
    """Parse and append every valid row of a gas readings CSV.
    ...
    Raises:
        GasMeterReadingError: If `path` does not exist or cannot be
    ...
    try:
        ...
        raise GasMeterReadingError(f"Cannot read CSV file '{csv_path}': {exc}") from exc
    ...
    with file:
        ...
        for row_number, row in enumerate(reader, start=2):  # header is row 1
            try:
                ...
                logger.error(f"Gas CSV import: skipping {message} in '{csv_path}'.")
...
class GasCsvSource(PersonalDataSource):
    """Manual-entry + CSV-import `PersonalDataSource` for `gas_consumption`.

    `collect()` reads every entry from this source's local log
    (written by `append_gas_reading()`/`import_gas_csv()`) and returns
    all of them, every time it is called -- it keeps no "already
    returned" cursor of its own. This is safe and idempotent by
    construction: EP-092's `PersonalDataManager`/
    ...
    def source_id(self) -> str:
        """Return this source's registration id ("gas_local_entry")."""
        return SOURCE_ID

    @property
    def category(self) -> str:
        """Return this source's category ("gas_consumption")."""
        return CATEGORY
        ...
        with self._log_path.open("r", encoding="utf-8") as file:
            for line_number, raw_line in enumerate(file, start=1):
                ...
                try:
                    ...
                    logger.error(
                        f"Gas local-entry log: skipping malformed line "
